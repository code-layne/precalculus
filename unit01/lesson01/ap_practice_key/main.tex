%! Functions and Function Notation — Unit 1, Lesson 1.1 — AP Practice (ANSWER KEY)
...
\item The function $W$ gives the water level of a reservoir, in feet, $t$ days after June 1.
      ...
      \begin{enumerate}[leftmargin=2.6em, itemsep=2pt]
        ...
        \item[{\color{keyred}$\checkmark$\,(B)}] Ten days after June 1, the water level is 42 feet.
...
\item If $f(x) = x^2 - 4x$, what is the value of $f(-3)$?
      ...
      \smallskip
      \textbf{(A)} $-21$ \hspace{2.2em} \textbf{(B)} $-3$ \hspace{2.2em} \textbf{(C)} $3$ \hspace{2.2em} {\color{keyred}\textbf{$\checkmark$\,(D)}} $21$

\item The function $p$ is defined by $p(x) = 3x + 1$ for $x < 4$, and $p(x) = x^2 - 6$ for
      $x \ge 4$. What is the value of $p(4)$?

      \smallskip
      {\color{keyred}\textbf{$\checkmark$\,(A)}} $10$ \hspace{2.2em} \textbf{(B)} $13$ \hspace{2.2em} \textbf{(C)} $23$ \hspace{2.2em} \textbf{(D)} Both 10 and 13
...
\item \begin{minipage}[t]{0.56\linewidth}
      ...
      \begin{enumerate}[leftmargin=2.6em, itemsep=3pt]
        \item[{\color{keyred}$\checkmark$\,(A)}] Domain $-4 \le x \le 3$; range $-2 \le y \le 4$
...
\item What is the domain of $g(x) = \dfrac{5}{2x - 8}$?
      \begin{enumerate}[leftmargin=2.6em, itemsep=2pt]
        ...
        \item[{\color{keyred}$\checkmark$\,(C)}] All real numbers except $4$
...
\item A shipping company charges a flat rate for light packages and a per-pound rate above
      ...
      units.
      \writespace{2.2cm}{$C(2) = 6$ (the first rule owns $w = 2$) and $C(5) = 6 + 2(5-2) = 12$. A package weighing 5 pounds costs \$12 to ship.}
      ...
      \writespace{2.4cm}{Only the first rule's condition includes 2: $0 < w \le 2$. The second requires $w > 2$, which 2 fails. Exactly one rule owns $w = 2$, so $C(2) = 6$ and nothing else.}
      ...
      \writespace{2.4cm}{No rule would own $w = 2$: the first stops just below it and the second starts just above it. $C(2)$ would be undefined, so 2 would have to be dropped from the domain --- and a 2-pound package would have no price.}
      ...
      \writespace{2.4cm}{$0 < w \le 10$; in brackets, $(\,0, 10\,]$. $C(12)$ has no meaning here: 12 is outside the domain, and the tariff defines no cost above 10 pounds.}
      ...
      \writespace{2.2cm}{$25$ for $10 < w \le 50$.}
