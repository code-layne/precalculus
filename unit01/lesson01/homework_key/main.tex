%! Functions and Function Notation — Unit 1, Lesson 1.1 — Homework (ANSWER KEY)
...
\item A phone plan charges a monthly fee plus a charge for each gigabyte of data:
      ...
      \writespace{1.1cm}{The monthly cost, in dollars, of a plan that uses 3 gigabytes of data that month.}
...
\item A courier charges a flat fee for short trips and a per-mile rate beyond that. For a
      ...
      \textbf{(a)} $D(2) = $ \ans{$4$} \hfill
      \textbf{(b)} $D(3) = $ \ans{$4$} \hfill
      \textbf{(c)} $D(9) = $ \ans{$13$}

      \textbf{(d)} Which rule owns $m = 3$, and how do you know? Name the condition you checked.
      \writespace{1.2cm}{The first rule owns it. Its condition is $0 < m \le 3$, and $m = 3$ passes that test; the second rule needs $m > 3$, which 3 fails.}

\item Let $v(x) = x^2 + 1$ for $x \le -2$, and $v(x) = 3 - x$ for $x > -2$.

      $v(-4) = $ \ans{$17$} \qquad $v(-2) = $ \ans{$5$} \qquad $v(0) = $ \ans{$3$}
...
\item A city pool charges \$5 admission for visitors aged 12 and under, and \$9 for visitors
      ...
      \writespace{2.0cm}{$P(a) = 5$ for $0 < a \le 12$, and $P(a) = 9$ for $12 < a \le 80$.}

\item Let $r(x) = \dfrac{12}{x + 4}$.

      \textbf{(a)} Find $r(2)$. \quad $r(2) = $ \ans{$2$}

      \textbf{(b)} Which input is forbidden, and why? Write the domain of $r$ in words.
      \writespace{1.4cm}{$x = -4$: it makes the denominator 0, which is undefined. The domain is every real number except $-4$.}
...
\item A classmate evaluates a piecewise function by working out \emph{both} rules and then
      ...
      \writespace{1.9cm}{Every input falls in exactly one condition, so exactly one rule is responsible for it. The conditions do the choosing, not the person evaluating --- the other rule was never an option for that input.}
