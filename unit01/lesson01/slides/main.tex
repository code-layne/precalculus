%! Functions and Function Notation — Unit 1, Lesson 1.1 — Slides (Beamer)
\documentclass[aspectratio=169, 11pt]{beamer}
\usepackage{precalculus-beamer}   % provides \plumheader{} and \sectionlabel[color]{}

\begin{document}

% TITLE SLIDE
{
\setbeamercolor{background canvas}{bg=plum}
\begin{frame}[plain]
  \vspace{2.8cm}\hspace{0.55cm}%
  \begin{minipage}{0.9\textwidth}
    {\color{goldacc}\bfseries\small LESSON 1.1}\\[0.5cm]
    {\color{white}\bfseries\LARGE Functions and Function Notation}\\[1.2cm]
    {\color{lilacmid}\small Precalculus~$\cdot$~Unit 1: Functions and Change}
  \end{minipage}
\end{frame}
}

% HOOK SLIDE
\begin{frame}[t, plain]
  \plumheader{Hook: The Machine Gets a Name}
  \sectionlabel[goldacc]{Think About It}

  A vending machine you can trust: press the same button, get the same item,
  every time. Give it a name --- let \(P(b)\) be the price in dollars of the item
  at button \(b\).

  \bigskip
  \begin{center}
    {\Huge\bfseries\color{plum} \(P(\text{B4}) = 1.25\)}
  \end{center}

  \bigskip
  \begin{columns}[T]
    \column{0.48\textwidth}
      \begin{block}{Say it out loud}
        What is this equation \textit{saying}? Give a full English sentence, not
        a reading of the symbols.
      \end{block}

    \column{0.48\textwidth}
      \begin{block}{Then ask}
        What is the \textbf{input} here? Does an input have to be a
        \textbf{number}?
      \end{block}
  \end{columns}

  \bigskip
  \textit{One short symbol just packed in the machine, the input, and the
  output. That is the whole point of function notation.}
\end{frame}

% SECTION 1 — FUNCTION NOTATION
\begin{frame}[t, plain]
  \plumheader{Function Notation --- Naming the Machine}
  \sectionlabel[goldacc]{Notes row NOTATION $\cdot$ I Do: problem 1 $\cdot$ We Do: problem 2}

  \begin{block}{Three parts, one symbol}
    In \(f(x)\): \quad \(f\) is the \textbf{name} of the function \(\cdot\)
    \(x\) is the \textbf{input} \(\cdot\) \(f(x)\) is the \textbf{output}.
  \end{block}

  \medskip
  Read \(f(x)\) as ``\textbf{\(f\) of \(x\)}.'' It is \textbf{not} \(f\) times
  \(x\) --- the parentheses hold the input, they do not multiply.

  \smallskip
  \begin{columns}[T]
    \column{0.48\textwidth}
      \textbf{In context.} A rideshare fare:
      \[ F(m) = 2.50 + 1.75m \]
      \begin{itemize}\setlength{\itemsep}{5pt}
        \item Name: \(F\)
        \item Input \(m\): miles driven
        \item Output \(F(m)\): the fare, in dollars
        \item So \(F(4)\) asks: what does a 4-mile ride cost?
      \end{itemize}

    \column{0.48\textwidth}
      \textbf{One statement, three facts.} \(f(3) = 7\) says:
      \begin{itemize}\setlength{\itemsep}{5pt}
        \item the input is \textbf{3}
        \item the output is \textbf{7}
        \item the point \(\mathbf{(3, 7)}\) is on the graph of \(f\)
      \end{itemize}
  \end{columns}
\end{frame}

% SECTION 2 — EVALUATING FROM A FORMULA
\begin{frame}[t, plain]
  \plumheader{Evaluating from a Formula}
  \sectionlabel[goldacc]{Notes row EVALUATING $\cdot$ I Do: problem 3 $\cdot$ We Do: problems 4--6}

  \begin{block}{The empty-slot habit}
    The slot does not care what you put in it:
    \[ f(x) = 5x - 8 \qquad\longrightarrow\qquad f(\square) = 5(\square) - 8 \]
    Write the parentheses \textbf{first}, empty. Then fill them.
  \end{block}

  \bigskip
  \begin{columns}[T]
    \column{0.31\textwidth}
      \textbf{Find \(f(3)\)}
      \begin{align*}
        f(3) &= 5(3) - 8 \\
             &= 7
      \end{align*}

    \column{0.31\textwidth}
      \textbf{Find \(f(-2)\)}
      \begin{align*}
        f(-2) &= 5(-2) - 8 \\
              &= -18
      \end{align*}

    \column{0.34\textwidth}
      \textbf{\(g(x) = x^2 + 3x\); find \(g(-4)\)}
      \begin{align*}
        g(-4) &= (-4)^2 + 3(-4) \\
              &= 16 - 12 = 4
      \end{align*}
  \end{columns}

  \bigskip
  \textbf{The trap:} \((-4)^2 = 16\), but \(-4^2 = -16\). Evaluating never
  changes the rule --- it just runs the machine once.
\end{frame}

% SECTION 3 — DOMAIN AND RANGE
\begin{frame}[t, plain]
  \plumheader{Domain and Range}
  \sectionlabel[goldacc]{Notes row DOMAIN \& RANGE $\cdot$ I Do: problem 7 $\cdot$ We Do: problems 8--10}

  \begin{block}{Two words for two sets}
    The \textbf{domain} is the set of all \textbf{inputs} the function uses. The
    \textbf{range} is the set of all \textbf{outputs} it produces --- a repeated
    output is listed \textbf{once}.
  \end{block}

  \medskip
  \begin{columns}[T]
    \column{0.48\textwidth}
      \textbf{From a list of points}
      \[ f = \{(-2, 6),\ (0, 4),\ (3, 6),\ (5, 1)\} \]
      Domain: \(\{-2, 0, 3, 5\}\) \\[3pt]
      Range: \(\{1, 4, 6\}\) --- the 6 is written once

      \medskip
      \textbf{From the graph on the last slide}\\[3pt]
      Domain: \(0 \le x \le 8\) \quad Range: \(1 \le y \le 7\)

    \column{0.48\textwidth}
      \begin{block}{Notation upgrade}
        \(0 \le x \le 8\) is also written \([\,0, 8\,]\) --- a square bracket
        includes the endpoint, a round one does not. Both forms count as
        correct.
      \end{block}

      \medskip
      \textbf{From a formula:} \(k(x) = \dfrac{8}{x + 3}\)

      \smallskip
      At \(x = -3\) the denominator is 0 --- \textbf{undefined}. Domain: all
      real numbers \textbf{except} \(-3\).
  \end{columns}
\end{frame}

% SECTION 4 — TWO DIRECTIONS
\begin{frame}[t, plain]
  \plumheader{Piecewise: One Function, Several Rules}
  \sectionlabel[redacc]{Notes row PIECEWISE --- the hinge of this lesson}

  A \textbf{piecewise function} is one function built from two or more rules,
  each rule owning its own stretch of inputs. The condition beside a rule is its
  \textbf{address}.

  \bigskip
  \begin{center}
    {\Large \(f(x) = \begin{cases} 2x & 0 \le x < 3 \\[2pt] 2 & x \ge 3 \end{cases}\)}
  \end{center}

  \bigskip
  \begin{columns}[T]
    \column{0.48\textwidth}
      \begin{block}{To evaluate}
        \textbf{1.} Find the stretch the input falls in.\\[3pt]
        \textbf{2.} Use \textit{only} that rule.\\[3pt]
        \textbf{3.} Evaluate it the usual way.
      \end{block}

    \column{0.48\textwidth}
      \begin{block}{Never}
        Run \textbf{both} rules and pick the answer you prefer. The condition
        has already picked for you --- one input, exactly one output.
      \end{block}
  \end{columns}
\end{frame}

% SECTION 4, CONT. — THE BOUNDARY (THE CRUX)
\begin{frame}[t, plain]
  \plumheader{The Boundary --- Reaching vs.\ Owning}
  \sectionlabel[redacc]{Notes row PIECEWISE $\cdot$ I Do: problem 11 $\cdot$ We Do: 12--14 (13 is the crux)}

  \begin{columns}[T]
    \column{0.46\textwidth}
      \begin{center}
      \begin{tikzpicture}[x=0.55cm, y=0.42cm]
        \draw[linegray, very thin, step=1] (0,0) grid (7,7);
        \draw[->, thick] (0,0) -- (7.6,0) node[below right] {\small \(x\)};
        \draw[->, thick] (0,0) -- (0,7.6) node[above] {\small \(f(x)\)};
        \foreach \x in {1,2,3,4,5,6,7} \node[below] at (\x,0) {\small \x};
        \foreach \y in {2,4,6} \node[left] at (0,\y) {\small \y};
        \draw[goldacc, very thick, dashed] (3,0) -- (3,7.2);
        \draw[very thick, plum] (0,0) -- (3,6);
        \fill[plum] (0,0) circle (2.5pt);
        \draw[very thick, plum, fill=white] (3,6) circle (2.5pt);
        \draw[very thick, plum] (3,2) -- (7,2);
        \fill[plum] (3,2) circle (2.5pt);
      \end{tikzpicture}
      \end{center}

    \column{0.50\textwidth}
      \begin{itemize}\setlength{\itemsep}{7pt}
        \item \(f(1) = ?\) --- which rule owns \(x = 1\)?
        \item \(f(5) = ?\)
        \item \(f(3) = ?\) --- the top rule \textbf{reaches} \(x = 3\), but its
              address stops at \(x < 3\)
        \item Same decision, no picture: \(g(x) = x^2\) for \(x \le -1\),
              \(g(x) = 4 - x\) for \(x > -1\). Find \(g(-3)\), \(g(-1)\).
      \end{itemize}
  \end{columns}

  \bigskip
  An \textbf{open circle} marks a value the rule gets close to but never reaches.
  A \textbf{filled dot} marks the one output the function actually has there.
\end{frame}

% YOU DO
\begin{frame}[t, plain]
  \plumheader{You Do --- Table \& Formula}
  \sectionlabel[redacc]{Notes row TABLE \& FORMULA $\cdot$ problems 15--18, alone, 13 minutes}

  \begin{enumerate}\setlength{\itemsep}{8pt}
    \setcounter{enumi}{14}
    \item Let \(f(x) = 3x - 7\). Find \(f(5)\) and \(f(-1)\). Fill the slot first.
    \item Using \(h\), find \(h(-1)\), \(h(2)\), \(h(6)\). Name the rule each time.

          \smallskip
          \(h(x) = \begin{cases} 5 - x & x < 2 \\[2pt] 2x & x \ge 2 \end{cases}\)
    \item From the \(s\) table: find \(s(4)\), then state the domain and range of \(s\).

          \smallskip
          \begin{tabular}{|c|c|c|c|c|c|}
          \hline
          \(x\) & 1 & 2 & 3 & 4 & 5 \\
          \hline
          \(s(x)\) & 12 & 9 & 9 & 6 & 3 \\
          \hline
          \end{tabular}
    \item A garage charges \$3 per hour for the first 2 hours, then a flat \$6 for
          any longer stay, up to 12 hours. Write \(C(h)\) as a piecewise function.
  \end{enumerate}
\end{frame}

% DEBRIEF
\begin{frame}[t, plain]
  \plumheader{Debrief --- The Headline}
  \sectionlabel[goldacc]{Share out 15--18, one answer each $\cdot$ then say it back}

  \begin{columns}[T]
    \column{0.48\textwidth}
      \begin{block}{The address}
        The condition beside a rule says \textbf{which inputs it is responsible
        for}.\\[3pt]
        No input belongs to two rules.
      \end{block}
    \column{0.48\textwidth}
      \begin{block}{The order}
        Decide \textbf{which rule owns the input} first.\\[3pt]
        \textit{Then} evaluate --- only that one.
      \end{block}
  \end{columns}

  \bigskip
  \textbf{Next time (1.2):} we stop running one machine at a time, and start
  combining two of them into a third.
\end{frame}

% HOMEWORK LAUNCH
\begin{frame}[t, plain]
  \plumheader{Start Your Homework}
  \sectionlabel[redacc]{Homework Launch $\cdot$ 3 minutes, alone, silent}

  \begin{itemize}\setlength{\itemsep}{8pt}
    \item Copy the due date into the \textbf{Homework} row of your cover sheet:
          \quad Due: \rule{3cm}{0.4pt}
    \item Start \textbf{problem 1} and \textbf{problem 4} now. On any piecewise
          question, name the \textit{condition} before you compute.
    \item The \textbf{Homework} (last two pages) is graded.
    \item \textbf{AP Practice} (the two pages before it) is \textbf{extra credit}
          --- optional, AP-exam style. Do the homework first.
  \end{itemize}
\end{frame}

\end{document}
