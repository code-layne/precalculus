...
\begin{frame}[t, plain]
  \plumheader{Two Directions: Evaluate vs.\ Solve}
  \sectionlabel[redacc]{Notes row TWO DIRECTIONS --- the hinge of this lesson $\cdot$ the direction table}
  ...
  \begin{block}{The headline}
    \textbf{Evaluate} hands you the input and asks for the output --- exactly one
    answer. \textbf{Solve} hands you the output and asks for the input(s) ---
    possibly several, and several is still a function.
  \end{block}

  \medskip
  Fill the direction table in the notes \textit{before} touching the graph. Then
  say, in your own words, how you tell the two questions apart before you compute.
\end{frame}

% SECTION 4, CONT. — THE GRAPH
\begin{frame}[t, plain]
  \plumheader{Reading Both Directions off a Graph}
  \sectionlabel[redacc]{Notes row TWO DIRECTIONS $\cdot$ I Do: problem 11 $\cdot$ We Do: 12--14 (12 vs.\ 13 is the crux)}
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{8pt}
    ...
    \item From the \(s\) table: find \(s(4)\), then solve \(s(x) = 9\). Which one
          was the \textit{evaluate} question?
          ...
          \hline
          \end{tabular}
    \item State the domain and range of \(p\) (the table with outputs 9, 6, 6, 3).
          Then run \(p\) \textit{backward}: is that a function?
    \item \(m(x) = \dfrac{20}{x - 6}\). Which input is forbidden, and why?
          Write the domain of \(m\) in words.
  \end{enumerate}
\end{frame}
  ...
  \begin{columns}[T]
    \column{0.48\textwidth}
      \begin{block}{Evaluate}
        Hands you the \textbf{input}, asks for the \textbf{output}.\\[3pt]
        Exactly \textbf{one} answer.
      \end{block}
    \column{0.48\textwidth}
      \begin{block}{Solve}
        Hands you the \textbf{output}, asks for the \textbf{input(s)}.\\[3pt]
        Zero, one, or \textbf{several} --- and several is still a function.
      \end{block}
  \end{columns}

  \bigskip
  \textbf{Next time (1.2):} not what the output \textit{is}, but what it
  \textit{does} as the input climbs.
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \begin{itemize}\setlength{\itemsep}{8pt}
    ...
    \item Start \textbf{problem 1} and \textbf{problem 4} now. Read the
          \textit{direction} of each question before you compute.
