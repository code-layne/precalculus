...
\item A phone is plugged in to charge, and its battery percent is checked every few minutes.
      ...
      Input: \blank{4.2cm} \qquad Output: \blank{4.2cm}
...
\item Write \textbf{yes} or \textbf{no}: is the rule a function?
      ...
      \hline
      each person $\to$ their age in years & \blank{1.2cm} &
      each age $\to$ a person that age & \blank{1.2cm} \\ \hline
      each word $\to$ its number of letters & \blank{1.2cm} &
      each teacher $\to$ a class they teach & \blank{1.2cm} \\ \hline
      \end{tabular}
...
\item Name each interval.
      ...
      Increasing: from \blank{2.8cm}

      Constant: from \blank{2.8cm}

      Decreasing: from \blank{2.8cm}
...
\item A parking garage charges by the hour, up to a daily maximum.
      ...
      \textbf{(a)} The cost is increasing from hour \blank{2.4cm}

      \textbf{(b)} The cost is constant from hour \blank{2.4cm}
