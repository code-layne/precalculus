%! Function Composition and Decomposition — Unit 1, Lesson 1.3 — Homework (ANSWER KEY)
...
\item \begin{minipage}[t]{0.40\linewidth}
      ...
      \textbf{(a)} $f(g(1))$: \; handoff $= $ \ans{$3$}, \; so $f(g(1)) = $ \ans{$5$}

      \textbf{(b)} $g(f(1))$: \; handoff $= $ \ans{$4$}, \; so $g(f(1)) = $ \ans{$4$}

      \textbf{(c)} $f(g(5))$ has no value. In one sentence, say what goes wrong with the handoff.
      \writespace{0.9cm}{$g(5) = 9$, and 9 is not one of $f$'s inputs in the table. The handoff lands outside $f$'s domain, so $f(9)$ has no value.}
...
\end{work}
      ...
      \textbf{(a)} $f(g(8))$: \; handoff $= $ \ans{$3$}, \; so $f(g(8)) = $ \ans{$12$}

      \textbf{(b)} $g(f(8))$: \; handoff $= $ \ans{$32$}, \; so $g(f(8)) = $ \ans{$27$}

      \textbf{(c)} Same two rules, same input. Are the two answers equal? \ans{No}
...
\end{work}
      ...
      \textbf{(a)} $(f \circ g)(x) = $ \ans{$x^2 - 6x + 11$} \qquad
      \textbf{(b)} $(g \circ f)(x) = $ \ans{$x^2 - 1$}

\item Let $f(x) = 5x + 1$ and $g(x) = 2x - 4$.

      \textbf{(a)} $(f \circ g)(x) = $ \ans{$10x - 19$} \qquad
      \textbf{(b)} $(g \circ f)(x) = $ \ans{$10x - 2$}
...
\item A step counter converts steps to distance: $K(s) = 0.0008s$ gives the kilometers walked in
      ...
      \writespace{1.2cm}{Handoff: $K(10{,}000) = 8$ kilometers. Then $C(8) = 480$, so walking 10,000 steps burns about 480 calories.}
...
\item A shipping desk converts a package's weight from ounces to pounds with
      ...
      \writespace{1.2cm}{$C$ expects pounds, and $P(48) = 3$ pounds, so $C(3) = \$9$ is a real price. $P$ expects ounces, but $C(48) = \$99$ is a price --- the handoff has the wrong units.}
...
\item Take each function apart into an inner rule and an outer rule.
      ...
      \textbf{(a)} $h(x) = (4x - 1)^5$: \quad inner $g(x) = $ \ans{$4x - 1$}, \quad
      outer $f(u) = $ \ans{$u^5$}

      \textbf{(b)} $k(x) = \dfrac{5}{2x - 9}$: \quad inner $g(x) = $ \ans{$2x - 9$}, \quad
      outer $f(u) = $ \ans{$5/u$}
...
\end{work}
      ...
      $(f \circ g)(x) = $ \ans{$x$} \qquad $(g \circ f)(x) = $ \ans{$x$}

      What do these two functions do to each other?
      \writespace{0.9cm}{They undo each other: each sends a number straight back out unchanged.}
...
\item A classmate says $f(g(x))$ and $g(f(x))$ must be the same function because they are built
      ...
      \writespace{1.2cm}{They are different functions because the handoff is different: $g$'s output goes into $f$ in one order, $f$'s output goes into $g$ in the other.}
