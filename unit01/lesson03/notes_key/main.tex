%! Function Composition and Decomposition — Unit 1, Lesson 1.3 — Guided Notes (ANSWER KEY)
...
%
% PAGE LOCKSTEP with notes/: the vocabulary write lines -> \ansline, \blank -> \ans, and
...
\ansline{Running two functions back to back: $(f \circ g)(x) = f(g(x))$.}\\[3pt]
\noindent\textbf{\textcolor{plum}{Inner and outer function:}}\\[1pt]
\ansline{In $f(g(x))$, $g$ (inner) runs first; $f$ (outer) runs on its result.}\\[3pt]
\noindent\textbf{\textcolor{plum}{Handoff value:}}\\[1pt]
\ansline{The number the inner function hands over --- its answer, the outer's question.}\\[3pt]
\noindent\textbf{\textcolor{plum}{Domain of a composite:}}\\[1pt]
\ansline{The inputs whose handoff $g(x)$ is a value the outer function accepts.}\\[3pt]
\noindent\textbf{\textcolor{plum}{Decomposition:}}\\[1pt]
\ansline{Writing one function as $f(g(x))$ --- composition run backwards.}
\end{vocabbox}
...
\ansline{No. Register 1's screen reads \$50 and it charges \$53.00; register 2's screen reads \$63.60 and it charges \$53.60. The totals differ by \$0.60 because the two middle numbers differ --- taxing first taxes the \$10 you never paid.}
\end{hookbox}
...
\end{center}
\vspace{2pt}
The handoff is measured in \labelbox{1.8cm}{feet}\qquad and $E$'s answer becomes $T$'s \labelbox{2.0cm}{question}
\\
& \notesprompt{Say the handoff out loud before you compute anything.}
\begin{probgrid}{|Y|Y|}
\pcell{1}{Name the handoff in $T(E(4))$, with its units. Then find $T(E(4))$.}{2.2cm}{The handoff is $E(4) = 800$ feet. Then $T(800) = 68 - 8 = 60$ degrees.} &
\pcell{2}{In $(T \circ E)(t)$, which function runs first? Then name one thing $(T \circ E)(t)$ does \emph{not} mean.}{2.2cm}{$E$ runs first --- the letter written last goes first. It does \emph{not} mean $T(t) \cdot E(t)$; a product has no handoff.} \\ \hline
\end{probgrid}
\\ \hline
...
$x$ & 0 & 3 & 4 \\
\hline
handoff: $g(x)$ & \ans{2} & \ans{0} & \ans{4} \\
\hline
$f(g(x))$ & \ans{0} & \ans{3} & \ans{1} \\
\hline
\end{tabular}
\end{center}
\\
...
\begin{probgrid}{|Y|Y|}
\pcell{3}{Find $f(g(1))$. Name the handoff first.}{2.0cm}{Handoff $g(1) = 3$, so $f(g(1)) = f(3) = 2$.} &
\pcell{4}{Find $g(f(1))$. Name the handoff first.}{2.0cm}{Handoff $f(1) = 4$, so $g(f(1)) = g(4) = 4$.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{5}{Try $f(g(2))$. What is the handoff, and why does the composite have no value here?}{2.2cm}{The handoff is $g(2) = 7$, and the table gives no $f(7)$. The outer function will not accept that question, so $f(g(2))$ has no value.} &
\pcell{6}{Find $f(g(3))$ and $g(f(3))$. Write both handoffs.}{2.2cm}{Handoff $g(3) = 0$, so $f(g(3)) = f(0) = 3$. Handoff $f(3) = 2$, so $g(f(3)) = g(2) = 7$.} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{7}{Build $(f \circ g)(x)$ for $f(x) = 3x-2$ and $g(x) = x^2+1$.}{2.2cm}{$f(g(x)) = 3(x^2+1) - 2 = 3x^2 + 3 - 2 = 3x^2 + 1$} &
\pcell{8}{Now build $(g \circ f)(x)$ for the same two functions. Is it the same expression?}{2.2cm}{$g(f(x)) = (3x-2)^2 + 1 = 9x^2 - 12x + 5$. Not the same --- not even the same shape.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{9}{Check $f(g(2))$ two ways: by naming the handoff, and from your answer to problem 7.}{2.2cm}{Handoff $g(2) = 5$, so $f(5) = 13$. From the formula, $3(2)^2 + 1 = 13$. Same number either route.} &
\pcell{10}{Let $p(x) = x^2$ and $q(x) = x - 4$. Build $(p \circ q)(x)$ and $(q \circ p)(x)$.}{2.4cm}{$(p \circ q)(x) = (x-4)^2 = x^2 - 8x + 16$, while $(q \circ p)(x) = x^2 - 4$.} \\ \hline
\end{probgrid}
\\ \hline
...
\end{center}
\vspace{2pt}
The two totals differ by \labelbox{1.6cm}{\$0.60} because the two \labelbox{2.2cm}{handoffs} differ
\\
...
\writespace{1.4cm}{Because the order decides which number gets handed over. The two orders build two different handoffs, and a different question to the outer function gives a different answer.}
\\
...
\begin{probgrid}{|Y|Y|}
\pcell{11}{Build $t(c(p))$ and $c(t(p))$ symbolically. By how much do the two rules differ at any price?}{2.4cm}{$t(c(p)) = 1.06(p - 10) = 1.06p - 10.60$, while $c(t(p)) = 1.06p - 10$. They differ by \$0.60 at every price.} &
\pcell{12}{Let $f(x) = 2x$ and $g(x) = x - 6$. Find $f(g(10))$ and $g(f(10))$. Write both handoffs and both answers.}{2.4cm}{Handoff $g(10) = 4$, so $f(g(10)) = 8$. Handoff $f(10) = 20$, so $g(f(10)) = 14$. One input, two orders, two answers.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{13}{Decompose $h(x) = \sqrt{x+5}$: give the inner rule and the outer rule, then check by substituting back.}{2.2cm}{Inner $g(x) = x + 5$; outer $f(u) = \sqrt{u}$. Check: $f(g(x)) = \sqrt{x+5} = h(x)$.} &
\pcell{14}{A classmate says $f(g(x))$ and $g(f(x))$ must match because they use the same two rules. Use the word \emph{handoff} to say what is wrong.}{2.2cm}{The rules are the same but the handoff is not. Each order hands a different number over, and the outer function answers whatever question it is given.} \\ \hline
\end{probgrid}
\\ \hline
...
\begin{probgrid}{|Y|Y|}
\pcell{15}{From the table: find $p(q(2))$ and $q(p(2))$. Write each handoff. Are the two answers equal?}{2.2cm}{Handoff $q(2) = 1$, so $p(q(2)) = p(1) = 5$. Handoff $p(2) = 3$, so $q(p(2)) = q(3) = 2$. Not equal.} &
\pcell{16}{Using $f$ and $g$ above, build $(f \circ g)(x)$ and $(g \circ f)(x)$, simplified. Same function or not?}{2.4cm}{$(f \circ g)(x) = x^2 + 3$, but $(g \circ f)(x) = (x+3)^2 = x^2 + 6x + 9$. Different functions.} \\ \hline
\end{probgrid}
\\
& \begin{probgrid}*{|Y|Y|}
\pcell{17}{Find $f(g(-2))$ by naming the handoff first, then check it with your formula from problem 16.}{2.2cm}{Handoff $g(-2) = (-2)^2 = 4$, so $f(4) = 7$. From $x^2 + 3$: $(-2)^2 + 3 = 7$.} &
\pcell{18}{Decompose $m(x) = (2x+1)^3$ into an inner rule and an outer rule. Which rule runs first when you compute $m(1)$? Find $m(1)$.}{2.4cm}{Inner $g(x) = 2x + 1$, outer $f(u) = u^3$. The inner rule runs first: the handoff is $g(1) = 3$, so $m(1) = 3^3 = 27$.} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
