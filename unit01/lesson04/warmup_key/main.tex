%! Inverse Functions — Unit 1, Lesson 1.4 — Warm-Up (ANSWER KEY)
...
\begin{enumerate}[label=(\alph*), itemsep=5pt]
  \item $f(g(11))$: \; handoff $= $ \ans{3}, \;
        so $f(g(11)) = $ \ans{11}
  \item $g(f(3))$: \; handoff $= $ \ans{11}, \;
        so $g(f(3)) = $ \ans{3}
...
\begin{enumerate}[label=(\alph*), itemsep=5pt]
  ...
  \item Read $q(x) = 3$ backwards and something goes wrong. The output 3 came
        from \textit{two} inputs: $x = $ \ans{1} and $x = $
        \ans{3}.
\end{enumerate}
