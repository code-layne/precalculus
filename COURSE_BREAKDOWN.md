...
**Per-lesson deliverables (7 PDFs + deck):** cover, warm-up + key, guided notes + key,
exit ticket + key, and a Beamer `slides/` deck. **There is no group activity** — the whole
gradual release runs inside the guided notes. **Homework is DeltaMath** — the cover carries
an assignment slot, a due-date slot, and its own score slot below the in-class total, and no
`homework/` directory exists unless a lesson is explicitly overridden because DeltaMath has
no practice for the topic. Lessons still on the pre-August shape ship an `activity/` +
`activity_key/` and a printed `homework/` + `homework_key/` until they are reauthored.

**Pedagogy:** traditional gradual release. Each 60-minute period runs warm-up → hook →
**I Do** → **We Do** → **You Do** → **debrief** → exit ticket, and the lesson plan opens
with a Lesson Flow box whose minutes sum to 60. I Do and We Do are the notes' `\S` sections;
You Do is the notes' practice box, and tiering happens as teacher circulation there rather
than on a separate sheet. The debrief is teacher-facing only; the student packet closes with
the exit ticket.
